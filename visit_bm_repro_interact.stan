data {
  ...
  real pregnant_interact[total_pools];
  real lactating_interact[total_pools];
  real postlactating_interact[total_pools];
}
...
parameters {
  ...
  real omega_pregnant_bodymass;
  real omega_lactating_bodymass;
  real omega_postlactating_bodymass;
}
...
model {
  for (i in 1:total_pools){
      ...
      omega_pregnant_bodymass * pregnant_interact[i] + omega_lactating_bodymass * lactating_interact[i] + omega_postlactating_bodymass * postlactating_interact[i] + 
  ...
  omega_postlactating_bodymass ~ normal(0, 2);
  omega_pregnant_bodymass ~ normal(0, 2);
  omega_lactating_bodymass ~ normal(0, 2);
  sigma ~ gamma(.2, 1);
  tau2 ~ normal(0, 2);
  tau3 ~ normal(0, 2);
}
